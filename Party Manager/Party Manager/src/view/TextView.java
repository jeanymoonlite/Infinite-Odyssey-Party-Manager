...
public interface TextView {
  ...
  /**
   * Renders a message listing out all the rules that this {@code TextView}'s {@code Manager} abides
   * by. This can include listing out the stats that {@code Character}'s use, if they have roles or
   * not, etc.
   *
   * @throws IOException if the transmission of the message to the data output fails
   */
   ...
   * the character's name, player name, the class (if applicable), class specification (if
   * applicable), health points, and the name of each stat and their values.
   *
   * @param name the name of the character to display
   * @throws IOException              if the transmission of the message to the data output fails
   * @throws IllegalArgumentException if there's no character with the given name in the manager. OR
   ...
   * @throws IOException              if the transmission of the message to the data output fails
   * @throws IllegalArgumentException if there's no party with the given name in the manager. OR if
   *                                  the given String is null
   * @throws IllegalStateException    if the manager has no parties
   */
  void displayParty(String name)
      throws IOException, IllegalArgumentException, IllegalStateException;

  /**
   * Renders every {@code Character} found in the {@code Manager}'s active party. This method
   * renders each character the same way that {@code displaysStatsOf()} does.
   *
   * @throws IOException           if the transmission of the message to the data output fails
   * @throws IllegalStateException if the manager has no parties
   */
